...
sf2 = [SaveFeatures(m) for m in [net[1], net[3], net[5], net[7]]]
...
#         torch.nn.utils.clip_grad_value_(net.parameters(), 10)
        ...
        if i % 50 == 49 :
            print('epoch = ', epoch + 1, ' step = ', i + 1, ' of total steps ', total_step, ' loss = ', round(loss.item() / 5, 4))
    ...
    if (val_acc * 100) > min_val :
        print('saving model')
        min_val = val_acc * 100
        torch.save(net.state_dict(), '../saved_models/4fm/model2.pt')
        
# checking accuracy of best model
net.load_state_dict(torch.load('../saved_models/4fm/model2.pt'))
...
plt.legend()
plt.savefig('../figures/4fm/training_losses2.jpg')
plt.close()

plt.plot(range(num_epochs), val_acc_list, 'r', label = 'validation_accuracy')
plt.legend()
plt.savefig('../figures/4fm/validation_acc2.jpg')
