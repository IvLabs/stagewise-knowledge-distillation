...
#             filename = '../saved_models/imagewoof/stage' + str(hyper_params['stage']) + '/model' + str(hyper_params['repeated']) + '.pt'
#         # separate if conditions for stage 1 and others because of irregular naming convention
#         # in the student model.
#         elif hyper_params['stage'] == 1 : 
#             filename = '../saved_models/imagewoof/stage0/model' + str(hyper_params['repeated']) + '.pt'
...
#             filename = '../saved_models/imagewoof/stage' + str(hyper_params['stage']) + '/model' + str(hyper_params['repeated']) + '.pt'
#         else : 
#             filename = '../saved_models/imagewoof/stage' + str(hyper_params['stage'] + 1) + '/model' + str(hyper_params['repeated']) + '.pt'
...
#                 torch.save(net.state_dict(), filename)
    ...
    net.cpu()
    filename = '../saved_models/imagewoof/stage5/model' + str(repeated) + '.pt'
    net.load_state_dict(torch.load(filename, map_location = 'cpu'))
    ...
    val_loss_list = list()
    min_val = 0
    savename = '../saved_models/imagewoof/medium_classifier/model' + str(repeated) + '.pt'
    for epoch in range(hyper_params["num_epochs"]):
        trn = []
        net.train()
        for i, (images, labels) in enumerate(data.train_dl) :
