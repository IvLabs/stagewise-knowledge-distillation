...
parser = argparse.ArgumentParser(description = 'Stagewise training of ResNet type model with Transfer Learning')
...
for repeated in range(0, 1) : 
    for stage in range(5) :
        ...
        if hyper_params['stage'] == 0 : 
            filename = '../saved_models/' + str(hyper_params['dataset']) + '/transfer_learning/' + str(hyper_params['model']) + '_stage' + str(hyper_params['stage']) + '/model' + str(hyper_params['repeated']) + '.pt'
        # separate if conditions for stage 1 and others because of irregular naming convention
        # in the student model.
        elif hyper_params['stage'] == 1 : 
            filename = '../saved_models/' + str(hyper_params['dataset']) + '/transfer_learning/' + str(hyper_params['model']) + '_stage0/model' + str(hyper_params['repeated']) + '.pt'
        ...
        project_name = 'tl-kd-' + hyper_params['model'] + '-' + hyper_params['dataset']
        experiment = Experiment(api_key="IOZ5docSriEdGRdQmdXQn9kpu", project_name = project_name, workspace="akshaykvnit")
        experiment.log_parameters(hyper_params)
        if hyper_params['stage'] == 0 : 
            filename = '../saved_models/' + str(hyper_params['dataset']) + '/transfer_learning/' + str(hyper_params['model']) + '_stage' + str(hyper_params['stage']) + '/model' + str(hyper_params['repeated']) + '.pt'
        else : 
            filename = '../saved_models/' + str(hyper_params['dataset']) + '/transfer_learning/' + str(hyper_params['model']) + '_stage' + str(hyper_params['stage'] + 1) + '/model' + str(hyper_params['repeated']) + '.pt'
    ...
    net.cpu()
    filename = '../saved_models/' + str(hyper_params['dataset']) + '/transfer_learning/' + str(hyper_params['model']) + '_stage5/model' + str(repeated) + '.pt'
    net.load_state_dict(torch.load(filename, map_location = 'cpu'))
    ...
    val_loss_list = list()
    min_val = 0
    savename = '../saved_models/' + str(hyper_params['dataset']) + '/transfer_learning/' + hyper_params['model'] + '_classifier/model' + str(repeated) + '.pt'
    for epoch in range(hyper_params["num_epochs"]):
        trn = []
        net.train()
        for i, (images, labels) in enumerate(data.train_dl) :
