...
#         torch.nn.utils.clip_grad_value_(net.parameters(), 10)
    ...
    if (val_acc * 100) > min_val :
        print('saving model')
        min_val = val_acc * 100
        torch.save(net.state_dict(), '../saved_models/medium_normal/model0.pt')
        
# checking accuracy of best model
net.load_state_dict(torch.load('../saved_models/medium_normal/model0.pt'))
...
plt.legend()
plt.savefig('../figures/medium_normal/training_losses_no_teacher0.jpg')
plt.close()

plt.plot(range(num_epochs), val_acc_list, 'r', label = 'validation_accuracy')
plt.legend()
plt.savefig('../figures/medium_normal/validation_acc_no_teacher0.jpg')
