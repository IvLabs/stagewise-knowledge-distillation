...
# project_name = 'trad-kd-ld-' + hyper_params['model'] + '-' + hyper_params['dataset']
...
# val_loss_list = list()
# min_val = 0
# savename = '../saved_models/' + str(hyper_params['dataset']) + '/trad_kd_ld/' + hyper_params['model'] + '_classifier/model' + str(hyper_params['repeated']) + '.pt'
...
for stage in range(1) :
    ...
    # project_name = 'trad-kd-ld' + str(args.p) + '-' + hyper_params['model'] + '-' + hyper_params['dataset']
    # experiment = Experiment(api_key="1jNZ1sunRoAoI2TyremCNnYLO", project_name = project_name, workspace="akshaykvnit")
    # experiment.log_parameters(hyper_params)

    filename = '../saved_models/' + str(hyper_params['dataset']) + '/trad_kd_ld' + str(args.p) + '/' + str(hyper_params['model']) + '_stage' + str(hyper_params['stage']) + '/model' + str(hyper_params['repeated']) + '.pt'
...
net.cpu()
filename = '../saved_models/' + str(hyper_params['dataset']) + '/trad_kd_ld' + str(args.p) + '/' + str(hyper_params['model']) + '_stage0/model' + str(hyper_params['repeated']) + '.pt'
net.load_state_dict(torch.load(filename, map_location = 'cpu'))
...
val_loss_list = list()
min_val = 0
savename = '../saved_models/' + str(hyper_params['dataset']) + '/trad_kd_ld' + str(args.p) + '/' + hyper_params['model'] + '_classifier/model' + str(hyper_params['repeated']) + '.pt'
for epoch in range(hyper_params["num_epochs"]):
    trn = []
    net.train()
    for i, (images, labels) in enumerate(data.train_dl) :
